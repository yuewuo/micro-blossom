...
def total_rounds(d, p): return int(100000 * ((7 / d) ** 3) * (0.001 / p))


if __name__ == "__main__":

    @slurm_distribute.slurm_distribute_run(os.path.dirname(__file__))
    def experiment(slurm_commands_vec=None, run_command_get_stdout=run_command_get_stdout):
        ...
        for p in p_vec:
            ...
            for idx, d in enumerate(d_vec):
                ...
                confidence_interval = math.sqrt(
                    1.96 * (offloading_rate * (1. - offloading_rate) / defect_num)) / offloading_rate
                print(
