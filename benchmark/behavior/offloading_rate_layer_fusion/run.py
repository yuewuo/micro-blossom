...
old_tmp_dir = os.path.join(script_dir, "..", "offloading_rate_pre_match", "tmp")
assert os.path.exists(old_tmp_dir), "run ../offloading_rate_pre_match/prepare.py first"
        ...
        for p in p_vec:
            ...
            for idx, d in enumerate(d_vec):
                ...
                syndrome_file_path = os.path.join(
                    old_tmp_dir, f"generated-d{d}-p{p}.syndromes"
                )
